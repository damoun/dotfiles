...
(defconst c-keywords 
  (sort 
   (list "if ()" "int" "char" "void" "NULL" "include" "define" "stdlib.h>" "unsigned" "while ()"
	"return ();" "struct" "union" "typedef" "float" "double" "sizeof()" "unistd.h>" "my_strlen()"
	"my_strcmp()" "my_strncmp()" "my_strcat();" "my_strncat();" "my_strcpy();" "my_strncpy();" "my_memset();"
	"my_printf();" "my_erprintf();" "my_getnbr()" "xmalloc()" "xcalloc()" "free()" "main ()" "break" "continue" "else"
	"static" "const" "signed" "extern" "malloc()" "exit();" "EXIT_FAILURE" "EXIT_SUCCESS" "size_t")
